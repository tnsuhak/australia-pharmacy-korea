...
(function(root){
  ...
  function classify(p,f){
    ...
    const check=(v,test,label)=>{if(v===null)unknown.push(label+' 확인 필요');else if(test(v))matches.push(label);else misses.push(label);};
    if(read(p.international_recruitment)!==true)unknown.push('2027 국제학생 모집 확인 필요');
    if(p.accreditation.status==='pending_2027')unknown.push('APC·Board 승인 확인 필요');
    const routeType=f.qualification==='graduate'?'graduate':f.route||'';
    const routes=p.entry_routes.filter(r=>!routeType||r.type===routeType);
    if(routeType&&routes.length===0)misses.push('검증된 해당 경로 미등록');
    if(routeType&&routes.length){
      if(!routes.some(r=>read(r.availability)===true))unknown.push('경로 연결조건 확인 필요');
      else matches.push(routeType==='graduate'?'Graduate Entry 경로':'선택 경로 연결');
    }
    const pathway=routeType&&routeType!=='direct';
    if(f.qualification&&f.qualification!=='graduate'){
      if(pathway)unknown.push('준비과정의 학력별 입학조건 확인 필요');
      else{
        ...
        else if(qv===null)unknown.push('학력별 Pharmacy 기준 확인 필요');
    ...
    if(f.chemistry==='no'){
      if(pathway)unknown.push('준비과정의 화학 이수·진급조건 확인 필요');
    ...
    if(f.intake){
      if(pathway){
        ...
        if(starts.some(x=>Array.isArray(x)&&x.includes(Number(f.intake))))matches.push('경로 시작월');
        else if(starts.some(x=>x===null))unknown.push('준비과정 시작월 확인 필요');
        else misses.push('경로 시작월');
      }else check(read(p.intakes.months),v=>v.includes(Number(f.intake)),'본과 시작월');
    }
    const years=read(p.duration_years);
    const duration=f.duration;
    if(duration){
      if(duration==='fast')check(years,v=>v<4||(p.id==='uq-bpharm-hons'&&f.intake!=='2'),'빠른 학위과정');
      else check(years,v=>v===Number(duration),'학위기간');
      if(pathway)unknown.push('준비과정 추가기간 별도');
    }
    if(f.structure){
      ...
      else if(f.structure==='3.75')check(years,v=>v===3.75,'3년 9개월 · 공개연도 확인');
      else check(years,v=>v===Number(f.structure),'세부 기간');
    }
    if(f.english){
      if(pathway)unknown.push('준비과정·진급 영어 별도 확인');
        ...
        if(p.english.ielts_overall.source_year&&p.english.ielts_overall.source_year<2027)unknown.push('영어 2027 재확인');
      }
    }
    if(f.cost){
      if(f.cost==='housing'){
        const confirmed=p.accommodation.filter(h=>read(h.weekly_cost)!==null);
        if(!confirmed.length)unknown.push('공식 숙소 요금 확인 필요');
        else if(confirmed.some(h=>read(h.weekly_cost)<=350))matches.push('숙소 주 A$350 이하 · 기준연도 확인');
        ...
        else if(p.scholarships.some(s=>read(s.pharmacy_eligible)===null&&s.assessment!=='not_eligible'))unknown.push('장학 적용 확인 필요');
    ...
    if(!routeType && misses.some(x=>['성적 기준','해당 학력의 Direct 평가 불가','화학 필수 아님'].includes(x))){
      ...
      if(alternatives.length){
        ...
        unknown.push('Direct 조건 미충족 · '+alternatives.map(r=>r.title).join(' / ')+' 별도 검토');
      }
    }
