...
def finder(home=False):
    ...
    advanced+=select('structure','학위·등록 구조',[('','전체'),('exit','4년 Exit 가능'),('integrated','Internship 학위 내 통합')])
...
home+='<section class="section finder-section"><div class="wrap"><div class="section-head"><div><span class="eyebrow">NOW FIND YOUR OPTIONS</span><h2>이제 내 조건으로 호주약대를 찾아보세요</h2><p>학력·화학 이수 여부·입학시기를 선택하면 검토할 경로를 좁혀드립니다.</p></div><a href="/compare/">상세필터로 전체 비교 →</a></div>'+finder(True)+'''<div class="quick-chips profile-chips"><span>내 상황 바로가기</span><a href="/foundation/">고2 수료 · Foundation</a><a href="/compare/?qualification=korean_high_school">고3 졸업 · Direct/Diploma</a><a href="/compare/?qualification=ged">검정고시 · 가능한 경로</a><a href="/admission-requirements/">수능·IB·A-level·OSSD</a></div></div></section>'''
...
comp+='<div class="compare-sticky"><a href="#finder">조건 수정 ↑</a><span>최대 3개 과정 비교</span></div><section class="section"><div class="wrap">'+finder()+'''<div class="results-head"><div><h2>검토할 과정 <span class="results-count" id="result-count">'''+str(len(P))+'''</span></h2><p id="result-summary" role="status" aria-live="polite">조건을 선택하면 확인 상태를 구분합니다.</p></div><p class="view-note">‘조건 일치’는 선택한 항목에만 해당합니다.<br>전체 입학자격·합격·비자 발급을 판정하지 않습니다.</p></div><div class="empty" id="empty-results" hidden><h3>확인된 조건에 맞는 과정이 없습니다</h3><p>조건을 줄이거나 아래 ‘추가 확인 필요’ 과정을 살펴보세요.</p><button type="button" id="reset-empty">필터 초기화</button></div><div class="card-grid" id="match-results">'''+''.join(card(p) for p in D['programs'])+'''</div><h2 class="pending-heading" id="pending-heading" hidden>추가 확인 필요 <span id="pending-count"></span></h2><p class="small" id="pending-note" hidden>미공개·자료 차이·경로별 세부 조건이 남아 있습니다. 불합격 또는 지원 불가를 뜻하지 않습니다.</p><div class="card-grid" id="pending-results"></div>'''+callout('UWA의 4년 Bachelor + Doctor of Pharmacy는 학부 시작 과정으로 포함했습니다. 별도 대학원 전용 과정과 CDU·UTS 등은 이 학부 비교에서 제외하며, 국제학생 모집 확인이 남은 과정은 ‘비교 후보’로 구분합니다.')+'</div></section>'
register('/compare/','2027 호주 약대 비교 · 입학경로·화학·입학시기 필터 | TNS',f'호주 약대 {len(U)}개 대학, {len(P)}개 과정을 조건별로 비교합니다. 3년·4년·5년, 화학, 7월 입학, Foundation·1학년 Diploma·Direct Entry를 구분합니다.',comp)
...
for u in D['universities']:
    ...
    items=[('overview','한눈에 보기',intro+'<h3>과정구조</h3>'+anatomy),('routes','입학방법',routecards(rs)),('admission','입학조건','<h3>Direct 학력·성적</h3>'+qtable(pid)+'<h3>선수과목</h3>'+requirements_html+'<h3>영어조건</h3>'+english_html+'<h3>입학시기</h3>'+fv(it['label'])+'<p class="small">Foundation·Diploma 시작월은 위 입학방법에서 별도로 확인합니다.</p>'),('cost','학비·장학금·생활비','<h3>학비</h3>'+fees+'<h3>장학금</h3>'+scholarcards(ss)+'<h3>기숙사·숙소</h3>'+housingcards(hh)+'<h3>총비용 참고</h3>'+costcalculator()),('registration','호주 약사등록',registration_html),('korea','한국 약사면허','<p>이 대학·과정에 대해 한국 면허 취득 가능 또는 보건복지부 인정 여부를 확정 표시하지 않습니다. 졸업 예정 학위명과 실제 교육과정, 현지 면허까지 포함해 별도 확인해야 합니다.</p>'+link('/korea-pharmacist/','한국 약사면허 확인 순서 →','btn text')),('faq','자주 묻는 질문',faq(fs)),('sources','자료 출처',sources(allids))]
...
pathway_items=[('overview','호주 약대 입학방법 한눈에 보기','<p>고교 단계에서 시작하는 대표적인 입학방법은 Foundation, 1학년 Diploma / IYO, Direct Entry 세 가지입니다. 어느 경로가 더 좋다는 뜻이 아니라 현재 학력·성적·영어·선수과목에 따라 출발점이 달라집니다.</p><div class="admission-grid"><article class="admission-card"><span class="route-label">FOUNDATION</span><h3>고2 수료부터 가능한 준비경로</h3><p>Foundation을 이수하고 약대 진급조건을 충족해 본과 1학년으로 진학합니다.</p><a href="/foundation/">Foundation 자세히 →</a></article><article class="admission-card"><span class="route-label">DIPLOMA / IYO</span><h3>대학 1학년 상당 과정 + 학점인정</h3><p>고교 졸업·검정고시 후 시작해 대학별 인정학점과 진급조건에 따라 본과로 연결합니다.</p><a href="/diploma/">Diploma 자세히 →</a></article><article class="admission-card"><span class="route-label">DIRECT ENTRY</span><h3>수능·IB·A-level·SAT·OSSD로 본과 지원</h3><p>약대 1학년으로 바로 지원하며 학력 환산점수, 선수과목, 영어를 함께 확인합니다.</p><a href="/admission-requirements/">Direct 입학조건 →</a></article></div>'),('profiles','내 상황에서는 어디부터 볼까요?',table([('고2 수료','Foundation 경로부터 확인'),('고3 졸업 예정','Direct와 Diploma를 함께 비교'),('검정고시','Diploma와 검증된 Direct 가능 경로 확인'),('수능·IB·A-level·SAT·OSSD','Direct Entry 조건 우선 확인')],['현재 상황','먼저 볼 경로'],True)),('graduate','대학 졸업자는 별도 특수경로','<p>Graduate Entry는 모든 호주 약대의 일반적인 네 번째 입학방법이 아닙니다. 관련 학사학위와 대학 수준 선수과목을 요구하는 대학별 특수경로로 별도 확인합니다.</p>'+link('/compare/?qualification=graduate','확인된 Graduate Entry 보기 →','btn text'))]
