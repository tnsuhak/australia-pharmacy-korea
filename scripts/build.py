...
STATUS={'confirmed_2027':'2027 공식 확인','latest_published':'최신 공개 기준','pending_2027':'2027 확인 중','source_conflict':'자료 차이 · 재확인'}
...
def header(path):
    links=[('/compare/','약대 비교'),('/admission-requirements/','입학조건'),('/foundation/','입학경로'),('/tuition-scholarships/','학비·장학금'),('/pharmacist-registration/','약사등록')]
...
def footer():return f'''<section class="consult-section" id="tns-channels"><div class="wrap"><div class="consult-intro"><div><span class="eyebrow">YOUR NEXT STEP</span><h2>내 성적표에서 시작하는 약대 계획</h2><p>학력·선수과목·영어·희망 시기를 정리하면<br>검토할 대학과 경로가 더 명확해집니다.</p></div><a class="btn secondary" href="/consult/">상담 전 준비할 내용 <span>→</span></a></div>{channels()}</div></section><footer class="site-footer"><div class="wrap"><div class="footer-top"><div><strong>TNS · 호주약대 가이드</strong><br><small>한국 학생을 위한 입학경로·과정 비교</small></div><nav class="footer-links" aria-label="하단 메뉴"><a href="/compare/">전체 약대</a><a href="/korea-pharmacist/">한국 약사면허</a><a href="/methodology/">자료 기준·업데이트</a><a href="/privacy/">개인정보 안내</a></nav></div><div class="footer-note">자료 확인일 {DATE} · 조건 충족 여부와 최종 입학허가는 대학 심사로 결정됩니다.<br>© TNS. TNS 유학 · 한국어 약대 정보·상담 서비스</div></div></footer>'''
...
def finder(home=False):
    ...
    first+=select('duration','희망 과정기간',[('','상관없음'),('fast','빠른 과정'),('4','4년'),('5','5년 통합')])
    advanced=select('route','입학경로',[('','전체 경로')]+list(MAIN_ROUTE.items()))
    advanced+=select('structure','과정구조',[('','전체'),('3','3년 Fast-track'),('3.5','3.5년'),('3.75','3년 9개월'),('4','4년'),('exit','4년 Exit 가능'),('integrated','Internship 통합')])
    advanced+=select('science','선수과목 비교',[('','전체'),('chemistry','화학 필수'),('mathematics','수학 필수'),('biology','Biology 인정')])
    advanced+=select('english','영어 기준',[('','전체'),('6.5','IELTS overall 6.5 이하'),('7','IELTS overall 7.0 이하'),('pte','PTE 기준 공개')])
    advanced+=select('cost','비용 조건',[('','전체'),('scholarship','한국학생 약대 적용 장학 확인'),('20','20% 이상 장학 검토'),('housing','공식 숙소 주 A$350 이하')])
    ...
    more=f'<details class="advanced"><summary>상세필터 · 입학경로, 영어, 비용</summary><div class="filter-grid advanced-grid">{advanced}</div><p class="filter-note" id="score-help">점수는 대학별 계산방식을 확인한 뒤 입력하세요. 영어 overall만으로 각 영역 충족 여부를 판정하지 않습니다.</p></details>'
    if home:more=''
    return f'''<form class="finder {'home-finder' if home else ''}" id="finder" action="/compare/" method="get" data-mode="{'navigate' if home else 'filter'}"><div class="finder-title"><h2>내 조건으로 호주약대 찾기</h2><p>간단한 조건부터 시작하세요</p></div><div class="filter-grid">{first}</div>{more}<div class="finder-actions"><p>미확정 조건은 ‘추가 확인 필요’로 구분합니다.<br>필터 결과는 합격 가능성 판정이 아닙니다.</p><div class="button-row">{'' if home else '<button type="reset" id="reset-filters">초기화</button>'}<button class="btn" type="submit">맞는 경로 살펴보기 <span aria-hidden="true">→</span></button></div></div></form>'''
def card(p):
    ...
    warn='<div class="notice-inline">신설 과정: APC 인증·Board 승인 대기</div>' if pid=='uq-pharmd' else '<div class="notice-inline">2027 국제학생 모집 확인 중 · 비교 후보</div>' if p['international_recruitment']['value'] is None else ''
...
home=f'''<section class="hero beginner-hero"><div class="wrap hero-grid"><div><span class="eyebrow">2027 AUSTRALIA PHARMACY GUIDE</span><h1>호주 약대,<br><em>처음부터 쉽게</em><br>알아보세요.</h1><p class="hero-copy">3년·4년·5년 과정부터 Foundation·Diploma·Direct까지.<br>먼저 호주 약대의 구조와 입학방법을 이해한 뒤<br>내 조건에 맞는 대학을 비교할 수 있습니다.</p><div class="button-row"><a class="btn" href="#pharmacy-basics">호주 약대 처음 알아보기 <span>↓</span></a><a class="btn secondary" href="#finder">내 조건으로 바로 찾기 →</a></div><div class="hero-meta"><span>{len(U)}개 대학 비교</span><span>3가지 주요 고교생 입학방법</span><span>Graduate Entry 별도 안내</span></div></div><div class="route-illustration summary-illustration"><div class="top"><h2>호주 약대 30초 요약</h2><span class="live-dot">BEGINNER GUIDE</span></div><div class="path-visual"><span class="n">3</span><div><h3>3년 Fast-track</h3><p>JCU · UTas</p></div><span class="end">압축과정</span></div><div class="path-visual"><span class="n">4</span><div><h3>기본은 4년</h3><p>일반 BPharm + UWA 4년 Bachelor·PharmD 예외</p></div><span class="end">가장 많음</span></div><div class="path-visual"><span class="n">5</span><div><h3>5년 통합형</h3><p>학위명·인턴 통합 여부가 대학마다 다름</p></div><span class="end">구조 확인</span></div></div></div></section>'''

home+='''<section class="section" id="pharmacy-basics"><div class="wrap"><div class="section-head beginner-head"><div><span class="eyebrow">START HERE</span><h2>호주 약대는 기본적으로 4년,<br>하지만 모두 같은 구조는 아닙니다</h2><p>기간만 보지 말고 학위 구조와 졸업 후 약사등록 과정을 함께 봐야 합니다.</p></div><a href="/pharmacist-registration/">약사등록 과정 자세히 →</a></div><div class="process-grid degree-overview"><article class="process-card"><span class="number">3<small>년</small></span><h3>Fast-track</h3><p><strong>JCU · University of Tasmania</strong><br>4년 상당의 학업을 3년에 압축해 이수합니다. 졸업 후 등록 인턴십은 별도입니다.</p><a href="/3-year-pharmacy/">3년 약대 비교 →</a></article><article class="process-card"><span class="number">4<small>년</small></span><h3>호주 약대의 기본 구조</h3><p>다수 대학은 4년 BPharm 계열입니다. <strong>UWA는 4년 안에 Bachelor + Doctor of Pharmacy를 함께 취득</strong>하지만 졸업 후 supervised internship은 별도인 예외적인 구조입니다.</p><a href="/universities/uwa-pharmacy/">UWA 구조 보기 →</a></article><article class="process-card"><span class="number">5<small>년</small></span><h3>5년 통합형도 서로 다릅니다</h3><p>Monash·Sydney·UNSW와 UQ의 신설 5년 과정처럼 Bachelor와 대학원 학위를 연결하지만, 인턴·실무훈련을 학위 안에 포함하는 방식은 서로 다릅니다.</p><a href="/compare/?duration=5">5년 과정 비교 →</a></article></div><div class="fact-strip"><strong>TNS 2027 비교 범위</strong><span>{len(U)}개 대학 · {len(P)}개 학부 시작 가능 과정</span><small>한국 국제학생이 고교 단계부터 검토할 수 있는 과정 중심이며, 모집·승인이 미확정인 과정은 별도로 표시합니다.</small></div></div></section>'''

home+='''<section class="section soft" id="entry-routes"><div class="wrap"><div class="section-head beginner-head"><div><span class="eyebrow">3 WAYS TO START</span><h2>고등학생이 호주 약대에 가는<br>대표적인 3가지 방법</h2><p>현재 학력과 영어 준비 정도에 따라 출발점이 달라집니다.</p></div></div><div class="admission-grid"><article class="admission-card"><div class="admission-no">01</div><span class="route-label">FOUNDATION</span><h3>Foundation → 약대 1학년</h3><p class="route-lead">고2를 마친 학생도 시작할 수 있는 준비과정이 있습니다.</p><dl><dt>대표 학력</dt><dd>고2 수료부터 가능한 과정 있음</dd><dt>영어</dt><dd>IELTS 약 5.0~5.5+부터</dd><dt>진학</dt><dd>진급조건 충족 → 약대 1학년</dd></dl><a href="/foundation/">Foundation 경로 보기 →</a></article><article class="admission-card"><div class="admission-no">02</div><span class="route-label">DIPLOMA / IYO</span><h3>1학년 Diploma → 학점인정</h3><p class="route-lead">고교 졸업·검정고시 후 대학 1학년 상당 과정을 먼저 이수합니다.</p><dl><dt>대표 학력</dt><dd>고3 졸업 또는 검정고시</dd><dt>영어</dt><dd>Pharmacy 연계는 IELTS 약 6.5 수준도 있음</dd><dt>진학</dt><dd>대학별 인정학점에 따라 본과 진학</dd></dl><a href="/diploma/">Diploma 경로 보기 →</a></article><article class="admission-card"><div class="admission-no">03</div><span class="route-label">DIRECT ENTRY</span><h3>Direct → 약대 1학년</h3><p class="route-lead">수능·IB·A-level·SAT·OSSD 등으로 본과에 바로 지원합니다.</p><dl><dt>대표 성적</dt><dd>수능 300점대부터 대학별 차이 큼</dd><dt>영어</dt><dd>IELTS 대체로 6.5~7.0</dd><dt>추가조건</dt><dd>수학·화학 등 선수과목 대학별 상이</dd></dl><a href="/admission-requirements/">Direct 조건 보기 →</a></article></div><p class="representative-note">위 조건은 호주 전체 대학의 공통 입학조건이 아니라 대표적인 출발 범위입니다. 실제 지원 가능 여부는 대학별 학력 환산, 선수과목, 영어 세부점수와 진급조건을 함께 확인해야 합니다.</p><div class="graduate-note"><div><span class="eyebrow">ALREADY HAVE A DEGREE?</span><h3>이미 대학을 졸업했다면 Graduate Entry도 있습니다</h3><p>관련 학사학위와 선수과목을 갖춘 지원자는 Monash Graduate Entry나 UWA의 별도 Doctor of Pharmacy 같은 대졸자 경로를 검토할 수 있습니다. 고교생 3가지 경로와는 따로 비교합니다.</p></div><a class="btn secondary" href="/compare/?qualification=graduate">Graduate Entry 보기 →</a></div></div></section>'''

home+='''<section class="section audience-section"><div class="wrap"><div class="section-head"><div><span class="eyebrow">WHERE DO I START?</span><h2>나는 어떤 경로부터 보면 될까요?</h2></div></div><div class="audience-grid"><a class="audience-card" href="/foundation/"><strong>고2를 마쳤어요</strong><span>Foundation부터 확인</span><b>→</b></a><a class="audience-card" href="/compare/?qualification=korean_high_school"><strong>고3 졸업 예정이에요</strong><span>Direct + Diploma 함께 비교</span><b>→</b></a><a class="audience-card" href="/compare/?qualification=ged"><strong>검정고시예요</strong><span>Diploma + 가능한 Direct 확인</span><b>→</b></a><a class="audience-card" href="/admission-requirements/"><strong>수능·IB·A-level·OSSD가 있어요</strong><span>Direct Entry 우선 비교</span><b>→</b></a></div></div></section>'''

home+='<section class="section finder-section"><div class="wrap"><div class="section-head"><div><span class="eyebrow">NOW FIND YOUR OPTIONS</span><h2>이제 내 조건으로 호주약대를 찾아보세요</h2><p>학력·화학 이수 여부·입학시기를 선택하면 검토할 경로를 좁혀드립니다.</p></div><a href="/compare/">상세필터로 전체 비교 →</a></div>'+finder(True)+'''<div class="quick-chips"><span>많이 찾는 조건</span><a href="/compare/?chemistry=no">화학을 안 배웠어요</a><a href="/compare/?intake=7">7월에 시작하고 싶어요</a><a href="/foundation/">Foundation으로 준비</a><a href="/diploma/">1학년 Diploma 학점인정</a></div></div></section>'''

home+='<section class="section soft"><div class="wrap"><div class="section-head"><div><span class="eyebrow">START WITH THESE OPTIONS</span><h2>구조가 다른 주요 약대부터 비교해 보세요</h2><p>3년 Fast-track, 4년 Bachelor+PharmD, 5년 통합형과 다양한 입학경로를 함께 봅니다.</p></div><a href="/compare/">'+str(len(U))+'개 대학 전체 보기 →</a></div><div class="card-grid">'+''.join(card(prog) for i in ['uwa','jcu','monash','uq','griffith','sydney'] for prog in D['programs'] if prog['university_id']==i and prog['id']!='uq-pharmd')+'</div></div></section>'

home+='''<section class="section pharmacist-intro"><div class="wrap"><div class="section-head beginner-head"><div><span class="eyebrow">BECOMING A PHARMACIST</span><h2>약대를 졸업하면 바로 약사가 되나요?</h2><p>학위 구조에 따라 실무훈련 시점은 다르지만, 일반등록까지의 절차를 확인해야 합니다.</p></div><a href="/pharmacist-registration/">호주 약사등록 자세히 →</a></div><div class="registration-steps"><div><b>01</b><strong>인증 약학과 졸업</strong><span>대학별 학위구조 확인</span></div><div><b>02</b><strong>Provisional Registration</strong><span>등록 실습 전 Board 요건 확인</span></div><div><b>03</b><strong>실무훈련·시험</strong><span>별도 internship 또는 학위 내 통합 여부가 다름</span></div><div><b>04</b><strong>General Registration</strong><span>최종 등록기준 충족 후 일반등록</span></div></div><div class="structure-callout"><strong>같은 PharmD라도 다릅니다.</strong><p>예를 들어 UWA의 4년 Bachelor + PharmD는 졸업 후 internship이 별도이고, Monash의 5년 PharmD는 5년차에 ITP와 유급 supervised practice를 포함합니다. UQ의 신설 5년 PharmD는 intern training을 학위 안에 설계했지만 현재 APC 인증·Pharmacy Board 승인을 진행 중입니다.</p></div></div></section>'''
...
register('/','2027 호주 약대 비교 · 내 조건에 맞는 입학경로 | TNS','한국 학생을 위한 2027 호주 약대 전문 가이드. Foundation, 1학년 Diploma, Direct Entry의 3가지 주요 입학방법과 선수과목·학비·약사등록 구조를 비교하세요.',home)

comp=pagehero('2027 호주 약대 비교',f'입학경로·선수과목·시작 시기를 함께 살펴보세요. {len(U)}개 대학의 {len(P)}개 과정 중 확인된 조건과 추가 확인이 필요한 항목을 나눠 보여드립니다.','전체 약대 비교')
comp+='<div class="compare-sticky"><a href="#finder">조건 수정 ↑</a><span>최대 3개 과정 비교</span></div><section class="section"><div class="wrap">'+finder()+'''<div class="results-head"><div><h2>검토할 과정 <span class="results-count" id="result-count">17</span></h2><p id="result-summary" role="status" aria-live="polite">조건을 선택하면 확인 상태를 구분합니다.</p></div><p class="view-note">‘조건 일치’는 선택한 항목에만 해당합니다.<br>전체 입학자격·합격·비자 발급을 판정하지 않습니다.</p></div><div class="empty" id="empty-results" hidden><h3>확인된 조건에 맞는 과정이 없습니다</h3><p>조건을 줄이거나 아래 ‘추가 확인 필요’ 과정을 살펴보세요.</p><button type="button" id="reset-empty">필터 초기화</button></div><div class="card-grid" id="match-results">'''+''.join(card(p) for p in D['programs'])+'''</div><h2 class="pending-heading" id="pending-heading" hidden>추가 확인 필요 <span id="pending-count"></span></h2><p class="small" id="pending-note" hidden>미공개·자료 차이·경로별 세부 조건이 남아 있습니다. 불합격 또는 지원 불가를 뜻하지 않습니다.</p><div class="card-grid" id="pending-results"></div>'''+callout('대학원 전용 UWA·CDU·UTS는 이 학부 비교에서 제외했습니다. La Trobe의 2027 국제학생 모집 등 확인 중인 과정은 ‘비교 후보’로 구분합니다.')+'</div></section>'
register('/compare/','2027 호주 약대 비교 · 입학경로·화학·입학시기 필터 | TNS','호주 약대 15개 대학, 16개 과정을 조건별로 비교합니다. 3년·4년·5년, 화학, 7월 입학, Foundation·1학년 Diploma·Direct Entry를 구분합니다.',comp)
...
def routecards(rs):
    html=''
    for r in rs:
        fields=[('연결 상태',fv(r['availability'])),('기간 / 시작',fv(r['duration'])+'<br>'+fv(r['intake'])),('본과 진급조건',fv(r['progression']))]
        if r['type']!='direct':fields.extend([('준비과정 입학 학력',fv(r['qualification'])),('준비과정 영어',fv(r['english']))])
        if r['credit']['value'] is not None:fields.insert(1,('인정학점',fv(r['credit'])+(' CP' if r['program_id'].startswith('griffith') else ' credits')))
...
def structure(pid):
    ...
    return facts([('전체 학위기간',fv(p['duration_label'])),('학사 취득 시점',fv(p['bachelor_award_year'],lambda f:str(f['value'])+'년차' if f['value'] else '확인 중')),('4년 Exit',fv(p['four_year_exit'])),('Exit 학위',fv(p['exit_degree'])),('최종 학위',fv(p['final_degree'])),('Supervised practice 포함',fv(r['supervised_practice_in_degree'])),('ITP 포함',fv(r['itp_in_degree'])),('졸업 후 별도 internship',fv(r['post_graduation_internship']))])
...
for u in D['universities']:
    ...
    extra='<div class="facts-grid">'+''.join(f'<div class="fact-tile"><small>{k}</small><strong>{v}</strong></div>' for k,v in [('과정',E(value(p['duration_label']))),('캠퍼스',E(value(u['campus']))),('입학시기',E(value(it['label']))),('국제학생 모집',E(value(p['international_recruitment']))),('연간 학비',E(money(t['annual']))),('확인일',DATE)])+'</div>'
    ...
    if p['review_items']:intro+=callout('<strong>추가 확인 항목</strong><ul>'+''.join('<li>'+E(x)+'</li>' for x in p['review_items'])+'</ul>',True)
    ...
    requirements_html=facts([(name,fv(rq[k])) for k,name in [('chemistry','Chemistry'),('mathematics','Mathematics'),('biology','Biology'),('physics','Physics'),('minimum_grade','최소 과목 성적 / 동등성')]])
    english_html=facts([(name,fv(en[k])) for k,name in [('ielts_overall','IELTS overall'),('ielts_bands','IELTS 영역별'),('pte_overall','PTE overall'),('pte_each','PTE each'),('toefl','TOEFL')]])+callout('영어 overall과 각 영역을 모두 확인해야 합니다. 입학 영어기준과 졸업 후 약사등록 영어기준도 별개입니다.')
    fees=facts([('연간 국제학생 학비',fv(t['annual'],money)),('수강량 기준',fv(t['load_basis'])),('대학 공식 indicative total',fv(t['official_total'],money))])+'<p class="small muted">'+E(t['increase_note'])+' 연간 학비 × 기간을 공식 총학비로 표시하지 않습니다.</p>'
    registration_html=facts([('APC 인증',fv(p['accreditation'])),('학위 내 supervised practice',fv(r['supervised_practice_in_degree'])),('학위 내 ITP',fv(r['itp_in_degree'])),('졸업 후 별도 인턴십',fv(r['post_graduation_internship']))])+callout(E(r['note']))+link('/pharmacist-registration/','약사등록 단계별 설명 →','btn text')
    fs=[('선수과목을 이수하지 않아도 지원할 수 있나요?',('JCU는 Chemistry를 권장 과목으로 안내합니다. 수학·영어 등 다른 입학요건은 별도로 충족해야 합니다.' if u['id']=='jcu' else '과정별 선수과목과 Foundation 등의 보완 경로를 함께 검토해야 합니다. 확인 중인 과목을 필수 없음으로 해석하지 마세요.')),
        ('2027 학비가 미공개인 항목은 어떻게 보나요?','기준연도가 확인된 수치만 표시합니다. 확인 중인 항목은 0원 또는 이전 연도 확정값으로 계산하지 않습니다.'),
        ('졸업하면 바로 호주나 한국 약사가 되나요?','호주는 등록용 실무·훈련·시험 및 일반등록 요건이 필요합니다. 한국은 별도의 대학 인정기준, 외국 면허, 예비시험·국가시험을 확인해야 합니다.')]
    ...
    items=[('overview','핵심 특징',intro),('structure','과정구조',anatomy),('routes','입학경로',routecards(rs)),('direct','Direct 입학조건',qtable(pid)),('prerequisites','선수과목',requirements_html),('english','영어조건',english_html),('intake','입학시기',fv(it['label'])+'<p class="small">Foundation·Diploma 시작월은 위 입학경로에서 별도로 확인합니다.</p>'),('tuition','학비',fees),('scholarships','장학금',scholarcards(ss)),('housing','기숙사·숙소',housingcards(hh)),('budget','총비용 참고',costcalculator()),('registration','호주 약사등록',registration_html),('korea','한국 약사 관련','<p>이 대학·과정에 대해 한국 면허 취득 가능 또는 보건복지부 인정 여부를 확정 표시하지 않습니다. 졸업 예정 학위명과 실제 교육과정, 현지 면허까지 포함해 별도 확인해야 합니다.</p>'+link('/korea-pharmacist/','한국 약사면허 확인 순서 →','btn text')),('faq','자주 묻는 질문',faq(fs)),('next','TNS 상담 준비','<p>성적표, 이수한 수학·과학 과목, 영어 각 영역, 희망 입학시기를 정리해 주세요.</p>'+link('/consult/','상담 준비하기 →','btn')+sources(allids))]
...
admit_items=[('read','입학조건은 네 가지를 따로 확인합니다','<div class="process-grid">'+''.join(f'<div class="process-card"><span class="number">{n}</span><h3>{t}</h3><p>{d}</p></div>' for n,t,d in [('1','학력과 성적','대학 최소 기준이 아닌 Pharmacy 전용 환산점수를 확인합니다.'),('2','선수과목','수학·화학·생물·물리의 과목명, 수준, 최소 성적을 대조합니다.'),('3','영어와 입학시기','약대 전용 영어의 각 영역과 해당 학기의 국제학생 모집을 확인합니다.')])+'</div>'+callout('SAT·IB·A-level이 있다고 모든 약대에 같은 점수로 지원할 수 있는 것은 아닙니다. 선수과목이나 영어조건이 점수와 별도로 적용됩니다.')),
 ('qualifications','한국 학생의 학력별 출발점',table([('수능','대학별 표준점수 합산 방식과 Pharmacy 환산 기준 확인'),('IB / A-level / SAT','학력 인정·시험 성적과 별도로 수학·과학 과목 수준 확인'),('OSSD','평균 산정 과목 수·Grade 12 과목·Pharmacy cut-off 확인'),('한국 내신 / 검정고시','대학별 인정 여부를 확인하고 Foundation·Diploma 입학자격도 함께 검토')],responsive=True)),
 ('scores','확인된 2027 Direct 수치', '<h3>Sydney · Pharmacy 과정 행</h3>'+qtable('sydney-bpharm-hons')+callout('다른 대학의 한국 수능·SAT·IB 기준은 course-specific 공식표를 대조 중입니다. Sydney 점수를 다른 약대에 적용하지 않습니다.',True)),
 ('latest-scores','Griffith · 이전 연도 참고 기준','<p>2026 International Guide에서 Pharmacy 1614는 H1 기준입니다. 아래 점수는 2027 확정값이 아니며 필터에서도 추가 확인 대상으로 분류합니다.</p>'+qtable('griffith-bpharm-hons')),
 ('prerequisites','선수과목·영어 핵심 비교',table(admission_rows,['과정','화학','수학','IELTS overall'],True)+callout('영어 영역별 기준은 대학 상세페이지에서 확인하세요. Assumed knowledge와 필수 prerequisite는 다르며, 확인 중은 ‘불필요’가 아닙니다.')),
 ('chemistry','화학을 이수하지 않았다면','<p>JCU는 Chemistry를 권장으로 안내하며 준비 과목을 제공합니다. 화학이 없어도 수학·영어·학력 요건은 따로 충족해야 합니다. 화학 필수 대학을 희망한다면 인정되는 Foundation 과목으로 보완 가능한지 검토하세요.</p>'+link('/compare/?chemistry=no','화학 미이수 조건으로 비교 →','btn')),
 ('next','다음 확인 단계','<p>현재 성적과 선수과목을 정리한 뒤, Direct와 준비과정을 비교해 보세요. 과목명 번역만으로 동등성을 판단하지 말고 필요하면 강의계획서까지 준비합니다.</p><div class="section-link-list"><a href="/foundation/">Foundation 진급조건</a><a href="/diploma/">Diploma 학점인정</a><a href="/consult/">상담 준비</a></div>'+sources(['sydney','griffith-2026','jcu-guide','uq','monash','qut-guide','rmit','newcastle','canberra-2027','unisq-course']))]
...
foundation_items=[('difference','Foundation은 무엇을 준비하나요?','<p>대학 입학에 필요한 학업·영어·과학 과목을 준비하는 경로입니다. 수료 후 약대 1학년으로 진학하는 구조가 일반적이므로, Diploma 학점인정과 구분하세요. Foundation 입학기준과 Pharmacy 진급기준은 각각 확인해야 합니다.</p><div class="route-mini"><span>Foundation 입학</span>→<span>지정 과목·영어 이수</span>→<span>약대 진급 기준 충족</span>→<span>본과 1학년</span></div>'),
 ('routes','대학별 Foundation 연결',routecards(foundation_routes)),
 ('uq','UQ · 2월 준비과정에서 7월 본과로','<p>UQ College Accelerated Foundation은 2027년 2월 15일 시작, 7월 9일 완료 일정입니다. 기존 UQ BPharm의 7월 26일 시작과 연결되는 달력 구조를 확인했습니다.</p>'+callout('달력상 연결 가능과 진학 보장은 다릅니다. Foundation 입학허가, 필수 과목 선택, GPA·영어, 최종 성적 처리 및 본과 오퍼·비자 일정이 모두 맞아야 합니다.')+'<p>BPharm 공개 진급 기준은 GPA 5.0, Academic English 5입니다. 새 UQ PharmD로 같은 조건이 적용된다고 가정하지 않습니다.</p>'),
 ...
 ('check','지원 전에 준비할 내용','<ul><li>고교 졸업/재학 증명과 학년별 성적표</li><li>희망 약대에 필요한 수학·과학 과목 조합</li><li>준비과정 영어조건과 본과 진급 영어조건</li><li>준비과정 시작일, 본과 시작일, 성적 발표일</li><li>준비과정과 본과의 학비·숙소를 나눈 예산</li></ul>'+sources(source_ids(foundation_routes)|{'uq-calendar','uq-foundation','uq'}))]
...
dip_items=[('credit','Diploma는 인정학점과 남은 기간을 봅니다','<p>과정 이름에 Health Sciences가 있어도 약대 2학년으로 연결된다고 단정할 수 없습니다. 공식 articulation에 적힌 대상 학위, 지정 과목, 인정학점, 진급 성적을 확인하세요. IYO도 해당 약대 연결이 검증된 경우에만 같은 범주에서 비교합니다.</p>'),('routes','확인된 연결과 별도 검토 경로',routecards(dip)),('griffith','Griffith · Direct와 College Diploma',table([('Direct','본과 1학년','고교/시험·선수과목·영어조건 충족'),('Griffith College Diploma','80CP 인정','본과 잔여 T1 3년 / T2 3.5년')],['경로','학점·진입','주의할 조건'],True)),('curtin','Curtin · 학점 외에 추가 조건','<p>현재 Curtin College 메인 과정 페이지는 Stage 2 CWA 70%를 안내합니다. 175 credits와 12월 PHAR1002 추가 이수를 함께 충족해야 합니다. 구 course information의 65%를 새 기준처럼 사용하지 않습니다.</p>'),('questions','자주 묻는 질문',faq([('Diploma를 마치면 무조건 2학년으로 가나요?','아닙니다. 대학 입학조건과 Diploma 진급 성적, 지정 과목, 영어조건을 충족하고 최종 학점인정을 받아야 합니다.'),('80CP는 항상 본과 3년을 뜻하나요?','Griffith College의 해당 협약은 T1 시작 시 3년, T2 시작 시 3.5년을 안내합니다. 시작학기에 따라 남은 일정이 달라집니다.')])+sources(source_ids(dip)))]
register('/diploma/','호주 약대 디플로마 · Griffith·Curtin 학점인정 | TNS','Griffith College와 Curtin College Pharmacy Diploma의 학점인정, 진급 성적, 추가 과목과 남은 학업기간을 비교합니다.',pagehero('1학년 Diploma로 연결하는 호주 약대','대학 1학년 상당 과정을 거친 뒤, 인정되는 학점과 실제 남은 기간을 확인하는 것이 중요합니다.','1학년 Diploma / IYO')+article(dip_items))
...
cost_items=[('tuition','대학별 국제학생 학비',table(fee_rows,['과정','연간 학비','공식 indicative total'],True)+callout('UTas는 연간 133 credits, UQ는 16 units 등 수강량 기준이 다릅니다. 공식 총액도 추정치이며 향후 학비 인상과 수강계획에 따라 달라질 수 있습니다.')),('scholarship-types','장학금은 심사 방식부터 구분','<div class="process-grid"><div class="process-card"><span class="number">01</span><h3>보장형</h3><p>명확한 보장 약정이 확인된 경우에만 사용합니다. 이번 검증 자료에서 일괄 보장형으로 분류한 장학은 없습니다.</p></div><div class="process-card"><span class="number">02</span><h3>자동심사형</h3><p>별도 장학 신청이 없어도 선발·자격 확인은 필요합니다. 오퍼에 수여가 명시됐는지 확인하세요.</p></div><div class="process-card"><span class="number">03</span><h3>경쟁형</h3><p>성적 상위 소수 선발입니다. 최대 장학률을 일반 학생 기본 비용에서 미리 빼지 않습니다.</p></div></div>'),('scholarships','주요 장학 조건',scholarcards([s for s in D['scholarships'] if s['university_id'] in ['griffith','sydney','monash','newcastle','unsw','jcu']])+callout('Sydney는 personal statement 제출이 필요합니다. UNSW 해당 20% 장학의 국가 목록에는 한국이 없어 한국 국적 기본 할인에 포함하지 않습니다. 나머지 대학은 상세페이지에서 적용 확인 상태를 볼 수 있습니다.')),('housing','숙소는 지역·계약기간까지 비교',housingcards([h for h in D['accommodation'] if h['weekly_cost']['value'] is not None])+'<p class="small">Sydney·UNSW 등 다른 도시의 검증된 공식 숙소 요금이 아직 부족해 도시 간 총비용 순위를 만들지 않았습니다. 보증금, 공과금, 식사와 계약기간을 같은 기준으로 맞춰야 합니다.</p>'),('calculator','내 가정으로 예산 계산',costcalculator()),('sources','자료 기준',sources(source_ids(D['tuition']+D['scholarships']+D['accommodation'])))]
...
reg_items=[('steps','호주 약사등록의 기본 흐름','<p>대학 실습과 등록을 위한 supervised practice는 같은 항목이 아닙니다. 통합학위에서는 아래 일부 단계가 재학 중 진행될 수 있습니다.</p><ol class="timeline">'+''.join(f'<li><strong>{t}</strong>{d}</li>' for t,d in [('승인된 학위·교육요건 확인','APC 인증과 Pharmacy Board 승인 학위명·캠퍼스를 확인합니다.'),('Provisional registration','등록용 supervised practice 시작에 필요한 등록 및 실습 승인 절차를 확인합니다.'),('Supervised practice + Intern Training','인정되는 감독 실무와 accredited intern training을 이수합니다.'),('Written / Oral examinations','응시요건 충족 후 등록시험을 진행합니다. 학위만으로 시험이 면제된다고 가정하지 않습니다.'),('General registration','모든 교육·실무·시험·등록 적합성 요건을 충족한 후 일반등록을 신청합니다.')])+'</ol>'+callout('이 페이지는 구조 설명입니다. 실습 시간, 시험 응시 시점, 등록 영어 등 수치 요건은 적용 시기의 Board 규정을 다시 확인해야 합니다. 현재 Board 페이지 접근 확인이 제한되어 시간 수치를 확정 게시하지 않습니다.',True)),('integrated','5년 통합 과정도 구조가 다릅니다',table([(link('/universities/monash-pharmacy/','Monash'),'4년 학사 Exit · 5년 통합','5년차 supervised practice / ITP 통합 · 등록시험·심사 별도'),(link('/universities/sydney-pharmacy/','Sydney'),'5년 BPharm(Hons) / MPharmPractice','실무·인턴 과정 포함 · 4년 Exit 조건 별도 확인'),(link('/universities/unsw-pharmacy/','UNSW'),'2027 PharmD 명칭 전환','현재 대학 안내상 학위 후 supervised internship 별도'),(link('/universities/uq-pharmacy/#uq-pharmd','UQ 신설 PharmD'),'기존 BPharm과 별도 5년 과정','인턴십 통합 설계 · APC/Board 승인 아직 미획득')],['과정','학위구조','등록 준비'],True)),('accreditation','인증 목록을 읽는 방법','<p>APC의 2026년 7월 8일 목록을 참고했습니다. Accredited with conditions는 교육 제공이 금지됐다는 뜻이 아닙니다. 다만 학위명·캠퍼스·조건·갱신 시점을 정확히 확인해야 합니다.</p><p>UQ 신설 통합 PharmD의 승인 대기 상태를 기존 BPharm 인증으로 대체하지 않습니다. UNSW의 새 PharmD 학위명 반영과 Newcastle의 2026년 말 인증기간 종료 이후 갱신도 확인 항목입니다.</p>'),('korea','한국 면허는 별도의 경로','<p>호주 일반등록과 한국 약사면허는 다른 심사 체계입니다. 한국으로 돌아갈 계획이라면 입학 전에 대학 인정기준과 외국 면허·시험 요건을 따로 확인하세요.</p>'+link('/korea-pharmacist/','한국 약사면허 경로 →','btn text')+sources(['apc','apc-exam','board','monash','sydney-guide','unsw','uq-pharmd']))]
...
register('/consult/','TNS 호주 약대 상담 준비 | 학력·성적·입학경로','TNS 호주 약대 상담 전 학력, 성적, 선수과목, 영어점수, 입학시기를 정리하고 상담 채널로 연결합니다.',pagehero('내 조건으로 상담 준비하기','현재 가능한 경로와 앞으로 준비할 조건을 나눠 검토합니다.','TNS 상담')+article(consult_items))
