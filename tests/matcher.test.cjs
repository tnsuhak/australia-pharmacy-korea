...
test('RMIT Associate Degree는 1년 Diploma가 아니라 2027 packaged 5년 경로',()=>{const r=catalog.entry_routes.find(x=>x.id==='rmit-associate');assert.equal(r.type,'other');assert.equal(r.availability.value,true);assert.equal(r.availability.status,'confirmed_2027');assert.match(r.duration.value,/총 5년/);assert.equal(r.credit.value,null);});
